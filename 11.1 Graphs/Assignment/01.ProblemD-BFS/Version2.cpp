...
int ShortestPath(std::vector<std::vector<int> >& graph, int number_of_nodes,
    ...
    std::vector<int> first_target_parent(number_of_nodes + 1, -1);
    std::vector<int> first_target_dist(number_of_nodes + 1, INT_MAX);
    std::vector<int> second_target_parent(number_of_nodes + 1, -1);
    std::vector<int> second_target_dist(number_of_nodes + 1, INT_MAX);
    std::vector<int> store_parent(number_of_nodes + 1, -1);
    std::vector<int> store_dist(number_of_nodes + 1, INT_MAX);

    BFS(graph, number_of_nodes, first_target, first_target_parent,
        first_target_dist);
    BFS(graph, number_of_nodes, second_target, second_target_parent,
        second_target_dist);
    BFS(graph, number_of_nodes, store, store_parent, store_dist);
